        ...
        while not self._stop_event.is_set():
            try:
                if self.cip.restart_connection is False:
                    ...
                    _logger.debug(
                        f'RX: <{str(binascii.hexlify(rx), "ascii")}>')
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if not self._stop_event.is_set():
                    ...
                    self.cip.socket.close()
                    _logger.debug(
                        f"lost connection to {self.cip.host}:{self.cip.port}")
                    time.sleep(10)
                else:
...
class CIPSocketClient:
    ...
    def unsubscribe(self, sigtype, join, direction="in"):
        with self.join_lock:
            if join not in self.join[direction][sigtype]:
                if sigtype == "s":
                    value = ""
    ...
    def register_callback(self, sigtype, join, callback):
        """ Allow callbacks to be registered for when dict entries change """
        self.subscribe(sigtype, join, callback)

    def remove_callback(self, sigtype, join):
        """ Allow callbacks to be de-registered """
        self.unsubscribe(sigtype, join)
